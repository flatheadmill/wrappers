...
#[wrappers_fdw(
    version = "0.1.0",
    author = "Suprabase",
    website = "https://github.com/supabase/wrappers/tree/main/wrappers/src/fdw/opensearch_fdw"
)]
pub(crate) struct OpenSearchFdw {
    // row counter
    row_cnt: i64,
...
impl ForeignDataWrapper for OpenSearchFdw {
    // 'options' is the key-value pairs defined in `CREATE SERVER` SQL, for example,
    //
    // create server my_opensearch_server
    //   foreign data wrapper wrappers_opensearch
    //   options (
    //     foo 'bar'
    // );
    //
